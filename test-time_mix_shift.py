...
from src.data.selectedRotateImageFolder import prepare_test_data
import warnings
...
def evaluate(cfg):
    ...
    for severity in severities:
        ...
        # for severity in severities:
            testset, _ = load_dataset(cfg.CORRUPTION.DATASET, cfg.DATA_DIR,
                                                cfg.TEST.BATCH_SIZE,
        ...
        test_loader = torch.utils.data.DataLoader(mixed_dataset,
                                                  batch_size = cfg.TEST.BATCH_SIZE,
                                                  shuffle = True,
                                                  num_workers = min(cfg.TEST.NUM_WORKERS, os.cpu_count()),
                                                  pin_memory = True)
        ...
        # accs.append(acc)
        logger.info(f"{cfg.CORRUPTION.DATASET} accuracy % [mixed_shift_{severity}][#samples={len(testset)}]: {acc:.2%}")
